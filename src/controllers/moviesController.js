        ...
        .then(movie =>{
            ...
            if(movie.length > 0){
                ...
                res.render('moviesDetail', {movie})
                 ...
                 async function Azul(request, response){
                     try{
                         console.log(request.body.titulo)
                    console.log("ACAAA")
                    let movie = await fetch(`http://www.omdbapi.com/?apikey=54cf1500&t=El+señor+de+los+anillos`)//.then(response => response.json())
                  //  let movie = await movieLook.json()
                   return response.render('moviesDetailOmdb', {movie})
                     }catch(error){
                        console.log(error)
                    }
